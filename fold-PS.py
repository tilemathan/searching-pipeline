...
def fold(filename,resultdir):
	...
	for i in range (1, numlines): #numlines for the whole file
		p = period[i]/1000
		d = dm[i]
		pd = p*a[i]/300000000 
        	os.system('prepfold -n 64 -nsub 64 -npart 8 -nodmsearch -noxwin -filterbank -p %s -dm %s -pd %s %s -o Cand%s' % (p, d, pd, filename, i))
	os.system('convert -adjoin *.ps %s-cands.pdf' % filename )
"""**************** MAIN PROGRAM *************************"""
