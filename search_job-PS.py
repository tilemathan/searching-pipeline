...
def create_shfiles(sourcename, P_orb, WD_mass, Min_mass):
	...
	for i in range(0, 4):
		os.chdir('/hercules/u/tilemath/PILOT-SURVEY-SPLIT/%s/%s_%s' % (sourcename,sourcename,PART[i]))
		for beam in range (0, 1):
			...
			os.system("mkdir /hercules/u/tilemath/PILOT-SURVEY-RESULTS/AC/%s_%s-results%s" % (sourcename, PART[i], beam))
			...
			f.write("srun search-PS.py /hercules/u/tilemath/PILOT-SURVEY-SPLIT/"+sourcename+"/"+sourcename+"_"+PART[i]+"/"+sourcename+"-"+"workdir"+beamstr+" "+"/hercules/u/tilemath/PILOT-SURVEY-RESULTS/AC/"+sourcename+"_"+PART[i]+"-"+"results"+beamstr+" "+"/hercules/u/tilemath/PILOT-SURVEY-SPLIT/"+sourcename+"/"+sourcename+"_"+PART[i]+"/"+sourcename+"_"+"0"+beamstr+"_"+PART[i]+".fil"+" "+P_orb+" "+WD_mass+" "+Min_mass)
			f.close()
			os.system("chmod +x"+" "+sourcename+"_0"+beamstr+".sh")
			os.system("sbatch %s_0%s.sh" % (sourcename, beamstr))
			os.chdir("..")
		os.chdir('/hercules/u/tilemath/PILOT-SURVEY-SPLIT/%s' % sourcename)
