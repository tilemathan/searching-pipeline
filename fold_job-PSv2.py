...
def create_shfiles(sourcename, P_orb, WD_mass, Min_mass):
	...
	for i in range(0, 4):
		for beam in range (0, 1):
			beamstr = str(beam)
			os.chdir('/hercules/u/tilemath/PILOT-SURVEY-RESULTS/AC/%s_%s-results%s/' % (sourcename, PART[i], beam))
			...
			f.write("srun fold-PSv2.py /hercules/u/tilemath/PILOT-SURVEY-SPLIT/"+sourcename+"/"+sourcename+"_"+PART[i]+"/"+sourcename+"_"+"0"+beamstr+"_"+PART[i]+".fil"+" "+"/hercules/u/tilemath/PILOT-SURVEY-RESULTS/AC/"+sourcename+"_"+PART[i]+"-results"+beamstr+"/ "+P_orb+" "+WD_mass+" "+Min_mass)
			f.close()
